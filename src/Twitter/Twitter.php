...
final class Twitter
{
    ...
    {
        if (get_class($request) === 'App\Twitter\Api\Request\Tweet') {
//            die(var_dump((
//            $this->connect()->get($request->uri())
//
//            )));
        }
        return Response\ResponseFactory::create($this->connect()->get($request->uri(), $request->parameters()), get_class($request));
    }
}